...
type Query {
  ...
  reviews: [Review]
  getReview(episode:Episode!): [Review]
...
/**
 * Allows us to fetch the ephemeral reviews
 */
function getReviews() {
  return reviews;
}
 ...
 */
function getReview(episode) {
  const reviewsByEpisode = reviews.filter((item) => item.episode == episode);
  return reviewsByEpisode;
}
/**
 * Allows us to update the ephemeral review
 */
...
/**
 * Allows us to delete the ephemeral review from list
 */
...
const resolvers = {
  Query: {
    ...
    getReview: (root, { episode }) => getReview(episode),
    reviews: (root) => getReviews(),
    search: (root, { text }) => {
  ...
  Mutation: {
    ...
    updateReview: (root, { id, review }) => updateReview(id, review),
    deleteReview: (root, { id }) => deleteReview(id),
  },
  Subscription: {
    reviewAdded: {
      subscribe: withFilter(
        ...
        (payload, variables) => {
          return (
            payload !== undefined &&
            (variables.review.episode === null ||
              payload.reviewAdded.review.episode === variables.review.episode)
          );
        }
      ),
    },
